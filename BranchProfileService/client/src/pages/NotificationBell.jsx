...
export function NotificationBell() {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch("http://localhost:3003/api/notifications"); // Update with your actual API endpoint
  ...
  const handleReject = async (id) => {
    try {
      await fetch(`http://localhost:3002/api/notifications/${id}/reject`, { method: "PATCH" });
      setNotifications((prev) =>
        prev.map((notification) =>
